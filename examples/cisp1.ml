...
let thread lst z d trig =
  ...
  trig |> pulseDivider n |> ofTrigger lst

let toTrigger = map Midi.isNoteOn
...
let midiFun input =
  ...
  mergeMidiStreams sqa sqb |> mergeMidiStreams sqc |> serialize |> map toRaw
