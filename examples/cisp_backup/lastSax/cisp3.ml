...
let currentState =
  (* init *)
  ref {c1= 0; p= Pitch 0; c2= 0; c3= 0; c4= 1; c5= 1; c6= 1; c7= 1; c8= 1}
...
let midiParser =
  ...
  let* (MidiVal ctrl8) = MidiState.getControlR (MidiCh 0) (MidiCtrl 8) in
...
let ofTrigger trig =
  ...
  let l8 = map (fun s -> s.c8) in
  let entryDelays =
    stt |> l4 |> map (fun s -> s |> float_of_int |> ( *. ) (0.01 /. 4.))
  in
  let combinedWalks =
    [ wlkr stt [|12; 7; 5|] 16.0 l1
    ; wlkr stt [|12; 7; 5|] 16.0 l2
    ; wlkr stt [|12; 7; 5; 7; 7; 7; 7; 7; 2|] 16.0 l3 ]
    |> List.to_seq |> transpose |> concat
  in
  let direction = seq [-1; 1] |> hold (st 3) in
  let arpSq =
    combinedWalks
    |> batcher (stt |> l6 |> ( +~ ) (st 1))
    |> map (fun steps -> walki 0 (direction *~ steps) |> ( +~ ) (stt |> l8))
    |> zipWith3
