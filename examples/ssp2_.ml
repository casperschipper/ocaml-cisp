...
let amp_array = walk 0.0 (lift rvf (-0.01) 0.01 |> hold (st 3)) |> Seq.map (clip (-1.0) 1.0) |> take 128 |> Array.of_seq 


let () = 
  let duras = rvf (rdRef bottomD) (rdRef topD) |> lookup_signal_array [|1;2;7;6;12;13;14;15|] in
  let holder = rvf (rdRef topAmp) (rdRef bottomAmp) |> lookup_signal_array [|1;2;3|] in 
