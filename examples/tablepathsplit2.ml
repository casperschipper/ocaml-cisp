...
let noise = 
  let open Cisp in
  lift rvf (-1.0) 1.0 |> Seq.map (fun x -> Float.pow x 10.0) |> hold (ch [|1;2;5;9;20;30;70|])|> take max |> Array.of_seq
...
let write_split () =
  ...
  let stream = 
    let open Cisp in
    seq [idx+1 mod max;0;0] |> hold (ch [|2;3;4;5;10;100;200|] |> hold (ch [|2;3;5;10;22;50|])) |> Infseq.cycleSq
...
let () = 
  ...
  let with_effect = ((effectSync eff (signal 0)) :: channels) in
   if false then
    ...
    Sndfile.write t "/Users/casperschipper/Music/Null/power_200_tozero.wav" Sndfile.WAV_32
  else 
