...
let edges_from_arrayarray arrr =
  ...
  |> Array.mapi (fun i arr ->
         arr |> Array.mapi (fun j strength -> { p1 = i; p2 = j; strength })) |> flatten
...
let svg_line p1 p2 edge min_strength max_strength =
  let x1, y1 = p1 in
  let x2, y2 = p2 in
  let thickness = normalize edge.strength min_strength max_strength 0.1 3.0 in
...
let generate_svg points pheromones =
  ...
  let svg_circles =
    Array.map svg_circle points |> Array.to_list |> String.concat ""
  in
  let svg_lines =
    edges
    |> Array.map (fun edge ->
           let p1 = points.(edge.p1) in
           (* pretty safe because map *)
           let p2 = points.(edge.p2) in
           svg_line p1 p2 edge min_strength max_strength)
    |> Array.to_list |> String.concat ""
  in
  (* Combine everything into the final SVG string *)
  [ svg_header; svg_circles; svg_lines; svg_footer ] |> String.concat ""
