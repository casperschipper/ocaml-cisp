...
val encode_osc_message :
  address:string -> args:osc_arg list -> bytes
...
val encode_osc_bundle :
  time:float -> messages:bytes list -> bytes
...
val send_bundle :
  sender -> time:float -> messages:bytes list -> unit
...
val simple_synth_with_pars :
  (string * osc_arg) list -> newSynth
