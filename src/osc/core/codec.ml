...
module Decode = struct
  ...
  let decode_timetag input =
    ...
    | 0l, 1l -> OscTypes.Immediate
    | _ -> OscTypes.(Time {seconds; fraction})

  let decode_argument input = function
    | 'f' -> Ok (OscTypes.Float32 (decode_float32 input))
    | 'i' -> Ok (OscTypes.Int32 (decode_int32 input))
    | 's' -> Ok (OscTypes.String (decode_string input))
    | 'b' -> Ok (OscTypes.Blob (decode_blob input))
    | 't' -> Ok (OscTypes.Timetag (decode_timetag input))
  ...
  let rec decode_bundle input =
    ...
    >>= (fun packets -> Ok OscTypes.({timetag; packets}))

  and decode_packet input =
    match decode_string input with
    | "#bundle" ->
      decode_bundle input >>= (fun bundle -> Ok (OscTypes.Bundle bundle))
    | address ->
      decode_arguments input >>=
      (fun args -> Ok (OscTypes.(Message {address = address; arguments = args})))
end
...
module Encode = struct
  ...
  let encode_argument output = function
    | OscTypes.Float32 f -> encode_float32 output f
    | OscTypes.Int32 i -> encode_int32 output i
    | OscTypes.String s -> encode_string output s
    | OscTypes.Blob b -> encode_blob output b
    | OscTypes.Timetag t -> encode_timetag output t

  let encode_arguments output args =
    let typetag_of_argument = function
      | OscTypes.Float32 _ -> 'f'
      | OscTypes.Int32 _ -> 'i'
      | OscTypes.String _ -> 's'
      | OscTypes.Blob _ -> 'b'
      | OscTypes.Timetag _ -> 't'
    in
  ...
  let rec encode_bundle output {OscTypes.timetag; packets} =
  ...
  and encode_packet output = function
    | OscTypes.Bundle bundle ->
      encode_string output "#bundle";
      encode_bundle output bundle
    | OscTypes.Message msg ->
      encode_string output msg.OscTypes.address;
      encode_arguments output msg.OscTypes.arguments
end
