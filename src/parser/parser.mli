type ('a, 'b) pstep = Good of 'a * char Seq.t | Problem of 'b * char Seq.t
...
val getRemainChars : (unit -> ('a, 'b) pstep) -> char Seq.t option
...
val oneOfParsers : ('a, 'b) parser list -> ('a, string) parser
type ('state, 'a) step = Loop of 'state | Done of 'a
val loopHelp :
  'a -> ('a -> (('a, 'b) step, 'c) parser) -> char Seq.t -> ('b, 'c) pstep
val loop : 'a -> ('a -> (('a, 'b) step, 'c) parser) -> ('b, 'c) parser
val parseZeroOrMore : ('a, 'b) parser -> ('a list, string) parser
...
val one_of : char list -> string -> (char, string) parser
val one_of_string : String.t -> string -> (char, string) parser
